            ...
            <div class="card bordewr-top-info p-4">
              <div class="card-body">
                <form action="{{route('sewa_mobil.update',$post->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <img class="logo-profil" src="{{asset('assets/foto profil/'.$post->foto_profil)}}" id="logo-image" width="180" height="180">
...
<div class="form-group mb-3">
    <div class="row">
        ...
        <input type="number" value="{{$post->no_kendaraan}}" class="form-control @error('no_kendaraan') is-invalid @enderror" name="no_kendaraan" placeholder="Nomor Kendaraan">
        @error('no_kendaraan')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
